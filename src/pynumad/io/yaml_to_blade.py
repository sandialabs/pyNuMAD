...
def yaml_to_blade(blade, filename: str, write_airfoils: bool = False):
    ...
    blade.sparcapwidth_hp = (
        np.array(blade_internal_structure["layers"][spar_hp_index]["width"]["values"])
        * 1000
    )
    blade.sparcapwidth_lp = (
        np.array(blade_internal_structure["layers"][spar_lp_index]["width"]["values"])
        * 1000
    )

    blade.sparcapoffset_hp = (
        np.array(
            blade_internal_structure["layers"][spar_hp_index]["offset_y_pa"]["values"]
        )
        * 1000
    )
    blade.sparcapoffset_lp = (
        np.array(
            blade_internal_structure["layers"][spar_lp_index]["offset_y_pa"]["values"]
        )
        * 1000
    )
    ...
    blade.leband = (
        np.array(blade_internal_structure["layers"][I_LE]["width"]["values"]) * 1000 / 2
    )
    blade.teband = (
        np.array(blade_internal_structure["layers"][I_TE]["width"]["values"]) * 1000 / 2
    )
    ### COMPONENTS
...
def _add_components(blade, blade_internal_structure, spar_hp, spar_lp):
    ...
    for comp in range(len(component_list)):
        ...
        if (
            "shell_skin_outer"
            in blade_internal_structure["layers"][comp]["name"].lower()
        ):
            component_list[comp].hpextents = ["le", "te"]
            component_list[comp].lpextents = ["le", "te"]
        ...
        if "web" in blade_internal_structure["layers"][comp].keys():
            if "fore" in blade_internal_structure["layers"][comp]["web"].lower():
                ...
                if (
                    ...
                    component_list[comp].name = blade_internal_structure["layers"][
                        comp
                    ]["name"]
                    # CK Change me when yaml is fixed!!!!
                    component_list[comp].cp[:, 1] = component_list[comp].cp[:, 1]
