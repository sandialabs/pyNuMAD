...
def writeBeamModel(wt_name, settings, blade, mu, log, directory="."):
        ...
        for filePath in glob.glob(directory + "/" + wt_name + "*.in"):
            ...
            try:
                ...
                while not licenseAvailable and nLicenceTries <= MAXnLicenceTries - 1:
                    subprocess.run(
                        this_cmd, shell=True, check=True, capture_output=True
                    )
    ...
    for fileName in glob.glob(directory + "/" + wt_name + "*." + extension):
        ...
        (
            beam_stiff[iStation, :, :],
            beam_inertia[iStation, :, :],
        ) = beamUtils.readVABShomogenization(fileName)

    if "beamdyn" in settings["beamSolver"].lower():
        beam_stiff, beam_inertia = beamUtils.transformMatrixToBeamDyn(
            beam_stiff, beam_inertia
        )
        axisFileName = beamUtils.write_beamdyn_axis(
            directory, wt_name, blade, radial_stations
        )
