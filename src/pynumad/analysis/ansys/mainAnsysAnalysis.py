...
def mainAnsysAnalysis(
    ...
    if (
        ...
        raise Exception(
            "Specify number of buckling modes when performing nonlinear buckling"
        )
    ...
    for iLoad in range(len(loadsTable)):
        ...
        nodeData = np.concatenate(
            [np.arange(meshData["nodes"].shape[0]).reshape((-1, 1)), meshData["nodes"]],
            axis=1,
        )
        ...
        ansys_call = (
            "export KMP_STACKSIZE=2048k & %s -b -p %s -I %s -o %s -np %s" % args
        )
        ...
        if (
            ...
            warnings.warn(
                "output designvar. Currently does not work for nonlinear cases"
            )
            ...
            wrinklingLimitingElementData = np.zeros(
                (len(linearLoadFactors), 4, len(imperfection))
            )
            marker = np.array(
                ["-ok", "-sk", "-dk", "-*k", "-^k", "-<k", "->k", "-pk", "-hk"]
            )
            ...
            for jj in range(len(imperfection)):
                for ii in range(len(linearLoadFactors)):
                    ...
                    wrinklingLimitingElementData[
                        ii, :, jj
                    ] = wrinklingForNonlinearBuckling(
                        blade,
            ...
            plt.title(
                "Imperfection Study (Linear Elements) SNL3p0-148-mk0p2-s1-fiberglass"
            )
    ...
    if (
        ...
        if not len(varargin) == 0 and class_(varargin[0]) == "IECDef":
            ...
            raise Exception(
                "IECDef required to run fatigue analysis in mainAnsysAnalysis"
            )
...
def saveData(
    designvar=None,
    iLoad=None,
    airfoilSegmentName=None,
    iSpan=None,
    nodes=None,
    midNodei=None,
):
    getattr[designvar.localFields[iLoad], [airfoilSegmentName]].x[iSpan] = nodes[
        midNodei, 1
    ]
    getattr[designvar.localFields[iLoad], [airfoilSegmentName]].y[iSpan] = nodes[
        midNodei, 2
    ]
    getattr[designvar.localFields[iLoad], [airfoilSegmentName]].z[iSpan] = nodes[
        midNodei, 3
    ]
    getattr[designvar.localFields[iLoad], [airfoilSegmentName]].data[iSpan] = nodes[
        midNodei, 4
    ]
    return designvar
