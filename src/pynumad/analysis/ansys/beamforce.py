...
def beamForceToAnsysShell(nodeData, loads, maptype="map3D_fxM0"):
    ...
    beta = [
        [0, -1, 0],  # Direction cosine matrix for a 90 degree clockwise rotation
...
def check_sums(nodeData, loads, forcemap):
    ...
    forcesums["RootMx"][:, 0] = np.multiply(
        -np.asarray(loads["rBlade"]), np.asarray(loads["Fyb"])
    ) + np.multiply(loads["prebend"], loads["Fzb"])
    forcesums["RootMy"] = np.zeros((len(loads["rBlade"]), 2))
    forcesums["RootMy"][:, 0] = np.multiply(
        np.asarray(loads["rBlade"]), np.asarray(loads["Fxb"])
    ) - np.multiply(loads["presweep"], loads["Fzb"])
    ## END
    ...
    for bk in range(len(loads["Fxb"])):
        ...
        forcesums["M"][bk, 1] = sum(
            np.multiply(-y.reshape(-1), forcemap["fx"][i])
            + np.multiply(x.reshape(-1), forcemap["fy"][i])
        )
        ## EMA original:
        ...
        x = nodeData[i, 0]
        y = nodeData[i, 1]
        forcesums["RootMx"][bk, 0] = sum(
            np.multiply(-z.reshape(-1), forcemap["fy"][i])
            + np.multiply(y.reshape(-1), forcemap["fz"][i])
        )
        forcesums["RootMy"][bk, 0] = sum(
            np.multiply(z.reshape(-1), forcemap["fx"][i])
            - np.multiply(x.reshape(-1), forcemap["fz"][i])
        )
        ## END
