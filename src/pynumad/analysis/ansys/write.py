...
def writeAnsysDeflections(blade, config, iLoad, fid, deflectionFilename):
    ...
    webSectionIDstart = (
        np.ceil(maxSectionNumber / 10**orderOfMagnitude) * 10**orderOfMagnitude
    )
    fid.write("/POST1\n" % ())
    fid.write("set,last\n" % ())
    fid.write("RSYS,0\n" % ())
    ...
    for i in range(blade.ispan.size):
        ...
        # fprintf(fid,'nplot\n');
        fid.write(
            "*GET, NsectionNodes, NODE,0,COUNT   !Get the number of nodes in the set\n"
            % ()
        )
        fid.write(
...
def writeAnsysFagerberWrinkling(app, SkinAreas, compsInModel, coreMatName):
    ...
    for kShearweb in range(TotalShearwebs):
        ...
        if coreMatName in mat.layer.layerName:
            ansysSecNumber = np.where(
                str(app.shearweb(kShearweb).Material) == str(compsInModel)
            )
    ...
    print(
        "\n\n The minimum wrinkling LF is: %f, wrinkle angle: %.2f°"
        % (minLF, LF[index, 3])
    )
    print(
...
def writeAnsysFatigue(fid, iLoad):
    ...
    fid.write(
        "PRETAB,zcent,eps11,eps22,eps12,kapa11,kapa22,kapa12,gamma12,gamma13,gamma23\n"
        % ()
    )
...
def writeANSYSinputFile(fid, mat, ansysSecNumber, coreMatName):
    ...
    fid.write(
        "*GET, enum, ELEM, 0, NUM, MIN, !  lowest element number in the selected set\n"
        % ()
    )
    ...
    fid.write(
        "enum =ELNEXT(enum)  !Next higher element number above N in selected set\n" % ()
    )
    fid.write("*ENDDO\n" % ())
    fid.write("\n" % ())
    fid.write("ALLSEL\n" % ())
    ...
    fid.write(
        "	    ETABLE,S11,S,X,AVG !AVG - Store averaged element centroid value\n" % ()
    )
    ...
    fid.write(
        "	ELNO=iel(J)  !It is needed to refer to ELNO in the command below\n" % ()
    )
...
def writeAnsysLinearBuckling(blade, config, iLoad, fid, bucklingFilename):
    ...
    fid.write(
        "bucopt,lanb," + str(config.analysisFlags.globalBuckling) + ",,,RANGE\n"
    ) % ()
...
def writeAnsysNonLinearBuckling(
    ...
    fid.write("/prep7\n" % ())
    fid.write(
        'UPGEOM,zImperfectionSF,1,%i,%s,"rst"\n'
...
def writeWrinklingForNonlinearBuckling(
    blade, coreMatName, settings, np, ansysFilename, i, j
):
...
def writeforcefile(filename, forcemap, forcesums, maptype):
    ...
    if "forcesums" is not None:
        ...
        fid.write("\n!Input          Fx =" % ())
        fid.write(
            "  %14.6e" * forcesums["Fx"][:, 0].size % tuple(forcesums["Fx"][:, 0])
        )
        fid.write("  %14.6e" % (sum(forcesums["Fx"][:, 0])))
        fid.write("\n!Output    sum(fx) =" % ())
        fid.write(
            "  %14.6e" * forcesums["Fx"][:, 1].size % tuple(forcesums["Fx"][:, 1])
        )
        fid.write("  %14.6e" % (sum(forcesums["Fx"][:, 1])))
        fid.write("\n!" + ("-" * len(s)))
        fid.write("\n!Input          Fy =" % ())
        fid.write(
            "  %14.6e" * forcesums["Fy"][:, 0].size % tuple(forcesums["Fy"][:, 0])
        )
        ...
        fid.write(
            "  %14.6e"
            * forcesums["RootMx"][:, 0].size
            % tuple(forcesums["RootMx"][:, 0])
        )
        fid.write("  %14.6e" % (sum(forcesums["RootMx"][:, 0])))
        fid.write("\n!Output  sum(z*fy) =" % ())
        fid.write(
            "  %14.6e"
            * forcesums["RootMx"][:, 1].size
            % tuple(forcesums["RootMx"][:, 1])
        )
        ...
        fid.write(
            "  %14.6e"
            * forcesums["RootMy"][:, 0].size
            % tuple(forcesums["RootMy"][:, 0])
        )
        fid.write("  %14.6e" % (sum(forcesums["RootMy"][:, 0])))
        fid.write("\n!Output  sum(z*fx) =" % ())
        fid.write(
            "  %14.6e"
            * forcesums["RootMy"][:, 1].size
            % tuple(forcesums["RootMy"][:, 1])
        )
...
def writeAnsysShellModel(blade, filename, meshData, config):
    ...
    for kmp in range(len(blade.materials)):
        ...
        if mat.type in ["isotropic", "orthotropic"]:
            ...
            fid.write(
                "\n   tbdata,1,%g,%g,%g,%g,%g,%g"
                % (uts[0], ucs[0], uts[1], ucs[1], uts[2], ucs[2])
            )
    ...
    for kStation in range(TotalStations):
        ...
        if isinstance(blade_struct["PresweepRef"]["pp"], int):
            ...
            presweep_slope = blade_struct["PresweepRef"]["pp"].__call__(
                blade.ispan[kStation], nu=1
            )
        if isinstance(blade_struct["PrecurveRef"]["pp"], int):
            precurve_slope = 0
        else:
            precurve_slope = blade_struct["PrecurveRef"]["pp"].__call__(
                blade.ispan[kStation], nu=1
            )
        ...
        transX = twistFlag * blade_struct["PresweepRef"]["pp"].__call__(
    ...
    if "281" == config["elementType"]:
        ...
        if "181" == config["elementType"]:
            ...
            raise Exception(
                "Element System %s not yet available", config["elementType"]
            )
    dup = []
    fid.write("\n! DEFINE ELEMENTS =======================================\n")
...
def write_ansys_loads(nodeData, loads, forcefilename, analysisConfig):
    ...
    if (
        ...
        forcemap, forcesums = beamForceToAnsysShellFollower(
            nodeData, loads, maptype=maptype
        )
    else:
